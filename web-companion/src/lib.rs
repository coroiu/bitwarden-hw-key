//! Library surface for the `web-companion` binary.
//!
//! Split out of `src/main.rs` (ai-bitwarden-hw-key-eml.7) so that a Phase-1
//! integration test (`tests/emulator_integration.rs`) can drive the REAL
//! `crate::transport::HttpEmulatorTransport` -- the exact CBOR-over-HTTP
//! client `POST /api/sync` uses in production -- against a running headless
//! emulator, without reimplementing the push logic or standing up the full
//! axum server + a real Bitwarden login. Integration tests under `tests/`
//! only see a crate's `pub` surface, which requires a library target;
//! before this bead, `web-companion` was binary-only (no `src/lib.rs`), so
//! nothing outside `src/main.rs` itself could reach `transport::
//! HttpEmulatorTransport` directly.
//!
//! `src/main.rs` is now a thin binary: it builds the initial [`state::AppState`]
//! and calls [`build_app`]/[`emulator_url`] from here. Nothing about the
//! server's behavior changes -- this is a pure code-motion refactor, not a
//! behavior change (see the moved `tests` module below, unchanged from its
//! original form in `main.rs`, all still passing).
...
#[must_use]
pub fn emulator_url() -> String {
    env::var("EMULATOR_URL").unwrap_or_else(|_| DEFAULT_EMULATOR_URL.to_string())
}

/// Builds the axum `Router`. Split out from `main` so tests (see the
/// `tests` module below) can construct the same app with a caller-supplied
...
mod tests {
    ...
    use super::*;
    use state::{Session, TransportRegistry};
