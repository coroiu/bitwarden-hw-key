...
- **Beads daemon can wedge on writes.** Symptoms: bd create / bd dep add time out on the .beads/bd.sock socket; 'sqlite3: database is locked'; 'Database out of sync with JSONL'. Seen causes: a stale long-lived bd daemon process, plus timed-out bd-create commands that get backgrounded and linger as zombies holding the SQLite lock. Recovery: pgrep -x bd then kill the stray processes (kill -9 is SQLite-safe since the DB rolls back incomplete transactions and .beads/issues.jsonl is the source of truth), rm -f .beads/bd.sock .beads/bd.sock.startlock, then bd --no-daemon sync --import-only to reconcile the DB from the JSONL. Prefer 'bd --no-daemon' for writes while flaky. Note: a supervisor gaming the Stop-hook's bead-comment check is a symptom of this, fix the daemon, do not game the hook.
- **sdkconfig.defaults must live at Cargo workspace root.** esp-idf-sys resolves sdkconfig.defaults relative to CARGO_WORKSPACE_ROOT at build time, not the firmware crate directory. After the workspace split, the firmware/sdkconfig.defaults path was silently dead (no error, just ignored). Symptom: config changes (e.g., stack size) had no effect. Fix: move sdkconfig.defaults to the workspace root.
- **T-Embed CC1101 variant display pinmap differs from plain T-Embed.** Not just the rotary encoder (GPIO4/5 vs different pins), but also the display: SPI SCLK GPIO11, MOSI GPIO9, CS GPIO41, DC GPIO16, backlight GPIO21 (plain code drove GPIO15, which is panel POWER-ENABLE on CC1101, leaving the backlight dark). Always verify pinouts for hardware variants.
- **mipidsi display_size must be the panel's native unrotated size.** Passing the logical rotated size (e.g., 320x170 for a 170x320 native panel) can exceed the framebuffer and cause InvalidDisplaySize boot-loops. Use native dimensions in display_size; apply Rotation separately.
...
-->
- **2026-08-17:** M0 (Platform Migration) COMPLETE. Physical T-Embed CC1101 board brought up on real hardware. All display, encoder, stack management, and feature work merged to main (merge commit ef3daf6). Epic `ai-bitwarden-hw-key-8d7` closed. M1.5 Phase 2 (device transport, overlaps M2) now unblocked. See `.planning/progress.md` for detailed M0 closure notes.
- **2026-08-11:** Migrated to a beads + multi-agent workflow (this file) and defined the 7-role team. M0 on-hardware validation work tracked in epic `ai-bitwarden-hw-key-8d7`.
